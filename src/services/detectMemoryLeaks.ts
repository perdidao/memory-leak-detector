...
export function startMemoryLeakAudit(
  ...
  function heapPercentage(): string {
    if (performance.memory) {
      return ((performance.memory.usedJSHeapSize / performance.memory.jsHeapSizeLimit) * 100).toFixed(2) + '%';
    }
    return 'N/A';
  }
  ...
  reportInterval = window.setInterval(() => {
    ...
    const listenerTypes = window.__eventAudit!.reduce((acc: Record<string, number>, curr: EventInfo) => {
      acc[curr.type] = (acc[curr.type] || 0) + 1;
      return acc;
    }, {});

    const typesWithMoreThanThreshold = Object.entries(listenerTypes).filter(([, count]) => count > typeOccurenceThreshold);
